...
window.app = Vue.createApp({
  ...
  data() {
    return {
      ...
      lnurlLink: `${window.location.host}/tagid/api/v1/scan/`,
      cards: [],
      hits: [],
      refunds: [],
      cardDialog: {
  ...
  methods: {
    ...
    getCards() {
      LNbits.api
        .request(
          'GET',
          '/tagid/api/v1/cards?all_wallets=true',
    ...
    getHits() {
      LNbits.api
        .request(
          'GET',
          '/tagid/api/v1/hits?all_wallets=true',
          this.g.user.wallets[0].inkey
        )
        .then(response => {
    ...
    getRefunds() {
      LNbits.api
        .request(
          'GET',
          '/tagid/api/v1/refunds?all_wallets=true',
          this.g.user.wallets[0].inkey
        )
        .then(response => {
    ...
    openQrCodeDialog(cardId, wipe) {
      ...
      this.qrCodeDialog.data = {
        id: card.id,
        link: window.location.origin + '/tagid/api/v1/auth?a=' + card.otp,
        encodedURI: encodeURIComponent(
          window.location.origin + '/tagid/api/v1/auth?a=' + card.otp
        ),
        name: card.card_name,
        uid: card.uid,
    ...
    createCard(wallet, data) {
      LNbits.api
        .request('POST', '/tagid/api/v1/cards', wallet.adminkey, data)
        .then(response => {
    ...
    enableCard(wallet, card_id, enable) {
      ...
      LNbits.api
        .request(
          'GET',
          '/tagid/api/v1/cards/enable/' + card_id + '/' + enable,
          fullWallet.adminkey
        )
        .then(response => {
